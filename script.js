...
function start() {
  ...
  allImageCopy = JSON.parse(JSON.stringify(allImages));
  ...
  for (let j = 1; j <= 16; j++) {
    ...
    randomImg = allImageCopy.splice(
      Math.floor(Math.random() * allImageCopy.length),
      1
    );
    ...
    div.addEventListener("click", function (event) {
      ...
      if (document.getElementsByClassName("match").length == 16) {
        ...
        let node = document.createTextNode("You won!!! Total Moves : " + moves);
        button.appendChild(node);
        document.getElementById("score").appendChild(button);
      }

      if (flag == 0) currImg.classList.add("showimg");
    });
  }
}
